<?php

class listcountry extends database {

    	private $array = array();
	private $table = 'listcountry';
	private $sql = false;
	private $status = false;

    	function get($fields = '', $conditions = '') {
		$this->sql = "SELECT $fields FROM `$this->table` $conditions";
		return $this->array = $this->result($this->sql);//calling parent inter method result which handles rest
    	}
    	function add( $_data = array() ) {

		return $this->insert_engine($this->table, $_data);
    	}
	function update( $array = array() ) {
		return $this->update_engine($this->table, $array);
	}
	function delete( $conditions = '') {
		return $this->delete_engine($this->table, $conditions);
	}
	function match_hash($idh = '') {
		return $this->match_hash_engine($this->table, $idh);
	}
}
